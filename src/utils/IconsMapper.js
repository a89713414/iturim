import * as icons from '@coreui/icons'

export const iconMap = {
  cilBell: icons.cilBell,
  cilCalculator: icons.cilCalculator,
  cilChartPie: icons.cilChartPie,
  cilCursor: icons.cilCursor,
  cilDescription: icons.cilDescription,
  cilDrop: icons.cilDrop,
  cilNotes: icons.cilNotes,
  cilPencil: icons.cilPencil,
  cilPuzzle: icons.cilPuzzle,
  cilSpeedometer: icons.cilSpeedometer,
  cilStar: icons.cilStar,
  cibCashapp: icons.cibCashapp,
  cibUblockOrigin: icons.cibUblockOrigin,
  cilInstitution: icons.cilInstitution,
  cilEnvelopeLetter: icons.cilEnvelopeLetter,
  cilHome: icons.cilHome,              // Added for Home
  cilHospital: icons.cilHospital,      // Added for Medical Rights
  cilShieldAlt: icons.cilShieldAlt,    // Added for Insurances
  cilDollar: icons.cilDollar,          // Added for Tax Refund
  cilBank: icons.cilBank,              // Added for Pension Withdrawals
  cilInfo: icons.cilInfo,              // Added for About Us
  cilContact: icons.cilContact         // Added for Contact Us
}
